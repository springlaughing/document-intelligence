...
// - after which a relay is free to publish at its leisure, and to retry.
//
// The consequence is at-least-once publishing: the relay can send and then fail before
// recording that it sent, so it will send again. Consumers must be idempotent.
public class OutboxMessage
{
    public Guid Id { get; set; }

    // Queue or topic to publish to. Stored per message so the relay stays ignorant of
    // what any particular message means.
    public string EntityName { get; set; } = default!;

    // Contract type name, carried so the relay can set ServiceBusMessage.Subject
